...
- Security controls (rate limiting, cron fail-closed auth, request body limit, outbound-fetch SSRF guard) live in `server.js` and are documented in `docs/features/security-hardening.md`. `CRON_SECRET` is required in production: when it is unset, `/api/cron` is rejected (fail-closed). AI/digest endpoints are rate limited per IP, per user, and globally.
